...
def framesighann(sig,frame_len,frame_step,winfunc=lambda x:numpy.ones((x,))):
    """Frame a signal into overlapping frames.
    ...
    frames = padsignal[indices]
    win = numpy.tile(winfunc(frame_len),(numframes,1))
    wind=numpy.tile(numpy.hanning(frame_len),(numframes,1))
    return frames*wind
    

def framesigblack(sig,frame_len,frame_step,winfunc=lambda x:numpy.ones((x,))):
    """Frame a signal into overlapping frames.
    ...
    frames = padsignal[indices]
    win = numpy.tile(winfunc(frame_len),(numframes,1))
    wind=numpy.tile(numpy.blackman(frame_len),(numframes,1))
    return frames*wind
